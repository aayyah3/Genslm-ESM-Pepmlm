...
from genslm_esm_training.dataset import GenSLMColatorForLanguageModeling
from genslm_esm_training.modeling_esm_v3 import EsmForContrastiveMaskedLM
from genslm_esm_training.modeling_esmc import ContrastiveEsmCConfig
from genslm_esm_training.modeling_esmc import EsmCForContrastiveMaskedLM
from genslm_esm_training.refseq_dataset import HDF5SequenceDataset
from genslm_esm_training.refseq_dataset import MultiEpochScannerSequenceDataset
from genslm_esm_training.refseq_dataset import ScannerSequenceDataset
...
class TrainingConfig:
    ...
    model_name: str = field(
        default=None,
        metadata={
            'help': 'Model designation choosing from ESMC_300M or ESMC_600M',
        },
    )
    ...
    compute_codon_loss: bool = field(
        default=True,
        metadata={'help': 'Whether to compute codon loss.'},
    )
    compute_aminoacid_loss: bool = field(
        default=True,
        metadata={'help': 'Whether to compute aminoacid loss.'},
    )
    compute_contrastive_loss: bool = field(
        default=True,
        metadata={'help': 'Whether to compute contrastive loss.'},
    )
    ...
    contrastive_pooler: str = field(
        default='mean',
        metadata={'help': 'Pooling strategy for contrastive loss.'},
    )
...
def main():
    ...
    if (
        config.model_name.lower() == 'esmc_300m'
        or config.model_name.lower() == 'esmc_600m'
    ):
        ...
        config.model_name.lower() == 'esm-650m'
        or config.model_name.lower() == 'esm-15b'
    ):
    ...
    model = ensure_uniform_dtype(
        model,
    )  # make sure we didnt put in mismatched weight types on accident
