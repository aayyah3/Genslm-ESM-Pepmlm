"""Utility functions for genslm-esm."""
...
def best_checkpoint(output_dir: Path) -> tuple[float, Path]:
    ...
    ckpt_dirs = sorted(ckpt_dirs, key=lambda x: int(str(x).split('-')[1]))
    ckpt_steps = [int(str(x).split('-')[1]) for x in ckpt_dirs]
    last_ckpt = ckpt_dirs[-1]

    # Load the trainer state with the full log history
...
def write_loss_curve(run_dir: Path, csv_file: Path) -> None:
    """Parse hugging face trainer state and write loss curve to CSV file.
    ...
    run_dir : Path
        The directory containing the training run checkpoints.
    csv_file : Path
        The file to write the loss curve to.
    """
    ...
    data: defaultdict[int, dict[str, Any]] = defaultdict(dict)
