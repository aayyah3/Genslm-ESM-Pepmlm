...
from genslm_esm_training.masking_scheduler import MaskingScheduler
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            with h5py.File(self.h5_files[file_idx], 'r') as data:
                ...
                print(
                    f'Index {idx} is out of bounds for dataset {data["input_ids"].shape[0]}',
                )
                ...
                for i, file in enumerate(self.h5_files):
                    with h5py.File(file, 'r') as data:
                        print(
                            f'[{i}] File {file} has {data["input_ids"].shape[0]} samples',
                        )
...
class RefSeqCollator:
    ...
    def __call__(self, batch):
        ...
        if self.masked_lm:
            ...
            tokenized['input_ids'][
                mask_indices & ~pad_indices
            ] = self.tokenizer.mask_token_id

            # reset 10% of the masked tokens to random tokens
            random_indices = mask_indices & (
                torch.rand(tokenized['input_ids'].shape) < 0.1 * mask_rate
            )
            tokenized['input_ids'][
                random_indices & ~pad_indices
            ] = torch.randint(4, 24, random_indices.size())[
                random_indices & ~pad_indices
            ]
...
if __name__ == '__main__':
    ...
    # )
    dataset = HDF5Dataset(
        '/lus/eagle/projects/FoundEpidem/azton/genslm2/genslm/hdf5_dataset_stdtables_directpqt/',
    )
    tokenizer = AutoTokenizer.from_pretrained(
        '/lus/eagle/projects/FoundEpidem/azton/genslm2/genslm/tokenizer_genomic_llama',
    )
    ...
    collator = RefSeqCollator(
        tokenizer=tokenizer,
        masked_lm=True,
        mlm_schedule='fixed',
        mlm_probability=0.3,
        require_tokenize=isinstance(dataset, ScannerSequenceDataset),
    )
    ...
    if not os.path.exists('aa_freq.txt'):
        for i, batch in tqdm(
            ...
            for s in input_ids:
                ...
                for aa in s:
                    if (
                        aa != tokenizer.pad_token_id
                        and aa != tokenizer.mask_token_id
                    ):
                        aminos[aa] += 1
        print(
            f'Found {bad_tok_seq_cnt} sequences with bad tokens of {(i + 1) * 128} samples ({bad_tok_seq_cnt / ((i + 1) * 128) * 100:0.2f}%)',
        )
