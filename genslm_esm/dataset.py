...
class StandardSampler(HDF5SequenceSampler):
    ...
    def __init__(
        self, file_path: Path, seed: int = 42, train_ratio: float = 0.9
    ) -> None:
...
class SequenceHomologySampler:
    ...
    def __init__(
        self, mmseqs_file: Path, train_ratio: float = 0.9, seed: int = 42
    ) -> None:
        ...
        md5_idx_to_h5_idx: dict[int, np.ndarray] = np.load(
            mmseqs_file, allow_pickle=True
        )
        ...
        for cluster_idx, cluster in md5_idx_to_h5_idx.items():
    ...
    def sample(self, idx: int, split: str = "train") -> int:
        """Sample a random sequence index from the `idx` cluster."""
        clusters = self.train_inds if split == "train" else self.eval_inds
        return np.random.choice(clusters[idx])


class CurriculumHDF5Dataset(Dataset):
    def __init__(
        self,
        h5_file: Path,
        sampler: HDF5SequenceSampler,
        split: str = "train",
        return_codon: bool = True,
        return_aminoacid: bool = False,
    ) -> None:
    ...
    def h5_data(self) -> h5py.File:
        ...
        if not hasattr(self, "_h5_data"):
            self._h5_data = h5py.File(self.h5_file, "r")
        return self._h5_data

    def __len__(self) -> int:
        return self.sampler.num_sequences(self.split)

    def __getitem__(self, idx: int):
        # Randomly sample one of the sequences within the `idx` sequence cluster
        sample_idx = self.sampler.sample(idx, split=self.split)
