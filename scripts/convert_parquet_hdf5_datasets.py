...
from genslm_esm.refseq_dataset import ScannerSequenceDataset
...
if False:
    for i, filename in tqdm(enumerate(train_path), total=len(train_path), position=0):
        ...
        h5path = "/lus/eagle/projects/FoundEpidem/azton/genomes/refseq_genslm_esm/test_{}.h5".format(
            rootname
        )
...
if True:
    ...
    h5path = "/lus/eagle/projects/FoundEpidem/azton/genomes/refseq_genslm_esm/valid{}.h5".format(
        rootname
    )
    with h5py.File(h5path, "w") as h5file:
        # h5file.create_dataset('id', data=ids, dtype=str_dtype)
