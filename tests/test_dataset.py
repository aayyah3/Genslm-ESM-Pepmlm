...
class TestGroupCodons:
    """Test cases for the group_codons function to ensure proper handling of ambiguous nucleotides."""

    def test_valid_codons_preserved(self):
    ...
    def test_ambiguous_nucleotides_replaced_with_unk(self):
        ...
        for ambiguous_char in ambiguous_tests:
            ...
            test_seq3 = f"A{ambiguous_char}G"  # A + ambiguous + G = invalid codon
            result3 = group_codons(test_seq3)
            assert result3 == "<unk>", f"Failed for {ambiguous_char} in middle"

    def test_mixed_valid_invalid_codons(self):
    ...
    def test_case_insensitive(self):
        """Test that the function handles both uppercase and lowercase input."""
    ...
    def test_sequences_not_divisible_by_3(self):
    ...
    def test_all_ambiguous_codons(self):
    ...
    def test_whitespace_handling(self):
        ...
        # The function groups by 3 characters, so spaces are treated as characters
        # This creates invalid codons: "A T", " G ", "Y R", " W " (last one incomplete)
        expected = "<unk> <unk> <unk> <unk>"
        assert result == expected

    def test_known_valid_codons_from_table(self):
        """Test that all codons in the translation table are recognized as valid."""
    ...
    def test_comprehensive_ambiguous_sequence(self):
        """Test a comprehensive sequence with multiple types of ambiguous characters."""
        # Create a sequence with various ambiguous characters
        complex_seq = "ATGYRWSKMDHVBNX-ATGCGCTAA"
        result = group_codons(complex_seq)
        # Expected: ATG (valid), YRW (invalid), SKM (invalid), DHV (invalid), BNX (invalid),
        # -AT (invalid), GCG (valid), CTA (valid), A (incomplete, invalid)
    ...
    def test_whitespace_in_output(self):
        ...
        assert result.count(" ") == 3  # Three spaces for four codons
